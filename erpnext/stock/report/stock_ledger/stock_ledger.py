...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		purchase_receipt_data = get_purchase_receipt_data(sle.item_code)
		purchase_order_data = get_purchase_order_name(sle.item_code)
		...
		if sle.item_code in item_details:
			item= item_details[sle.item_code]
			sle.update({
				"supplier": purchase_receipt_data.get('supplier_name'), 
                "purchase_order_id": purchase_order_data ,  # Replace with the actual fieldname for PO ID in your Item
                "invoice_id": item.get("invoice_id"),  # Replace with the actual fieldname for Invoice ID in your Item
                "invoice_date": item.get("invoice_date"),  # Replace with the actual fieldname for Invoice Date in your Item
...
def get_purchase_receipt_data(item_code):
    # Define the filter criteria to find the relevant "Purchase Receipt" document
    filters = {
        "docstatus": 1,  # Consider only submitted documents
        "item_code": item_code,
    }

    # Fetch the most recent "Purchase Receipt" document that matches the criteria
    purchase_receipt = frappe.get_list(
        doctype="Purchase Receipt",
        fields=["supplier", "posting_date"],
        filters=filters,
        order_by="posting_date DESC",  # Get the most recent receipt
        limit=1,  # Limit to one document
    )

    if purchase_receipt:
        # Extract the relevant data from the fetched document
        data = purchase_receipt[0]
        supplier_name = frappe.get_value("Supplier", data["supplier"], "supplier_name")
    ...
    # If no matching "Purchase Receipt" document is found, return an empty dictionary
    return {}

def get_purchase_order_name(item_code):
	filters={
		"item_code": item_code
	}
