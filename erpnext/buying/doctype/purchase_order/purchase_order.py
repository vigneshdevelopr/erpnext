...
class PurchaseOrder(BuyingController, Document):
    ...
    def autoname(doc):
        ...
        suffix = count_doc.counter

        # if self.docstatus == 2:
        # 	self.name = f'AGKL/PUR/ORD/{suffix}/REV:'
        # else:
        doc.name = f"AGKL/PUR/ORD/{suffix}"
        frappe.msgprint("counted")

    # 		if self.amended_from:
    # 			self.name = f'{doc.name}/REV:'
    # 			frappe.msgprint(doc.name)

    # def autoname(self,method):
    # 	if method == 'on_update' and self.docstatus == 1 and self.amended_from:
    # 		if not self.name.endswith('/REV'):
    # 			self.name = f'{self.name}/REV'

    # 	if method == 'on_submit' and not self.amended_from:
    # 		count_doc = frappe.get_doc('purchase-counter-suffix','counter')
    # 		count_doc.counter += 1
    # 		count_doc.save()
    # 		suffix = count_doc.counter
    # 		self.name = f'AGKL/PUR/ORD/{suffix}'

    # doc_events = {
    # 	'Purchase Order': {
    # 		'on_update': custom_autoname,
    # 		'on_submit': custom_autoname
    # 	}
    # }

    # frappe.get_doc("Purchase Order").before_insert.append(autoname)

    # def before_validate(doc):
    # 	if doc.amended_from and not doc.name.endswith('/REV:'):
    # 	#    doc.name =  f'/REV:{doc.name}'
    # 		 doc.name = f'{doc.name}/REV:'
    # # frappe.get_doc('Purchase Order').before_save.append(before_save)

    def onload(self):
        supplier_tds = frappe.db.get_value(
    ...
    def validate_supplier(self):
        ...
        if prevent_po:
            standing = frappe.db.get_value(
                "Supplier Scorecard", self.supplier, "status"
            )
            if standing:
                frappe.throw(
                    _(
                ...
                for item in self.items:
                    if not item.fg_item:
                        ...
                        if not frappe.get_value(
                            "Item", item.fg_item, "is_sub_contracted_item"
                        ):
                            frappe.throw(
                                _(
                                    "Row #{0}: Finished Good Item {1} must be a sub-contracted item"
                                ).format(item.idx, item.fg_item)
                            )
        ...
        for d in self.get("items"):
            if d.item_code:
                ...
                if last_purchase_details:
                    ...
                    d.base_rate = last_purchase_details["base_rate"] * (
                        flt(d.conversion_factor) or 1.0
                    )
    ...
    def check_modified_date(self):
        mod_db = frappe.db.sql(
            "select modified from `tabPurchase Order` where name = %s", self.name
        )
        date_diff = frappe.db.sql(
            "select '%s' - '%s' " % (mod_db[0][0], cstr(self.modified))
        )
            ...
            for d in self.supplied_items:
                if d.rm_item_code:
                    ...
                    stock_bin.update_reserved_qty_for_sub_contracting(
                        subcontract_doctype="Purchase Order"
                    )
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
    def postprocess(source, target):
        ...
        target.credit_to = get_party_account(
            "Supplier", source.supplier, source.company
        )
    ...
    def update_item(obj, target, source_parent):
        ...
        target.qty = (
            target.amount / flt(obj.rate)
            if (flt(obj.rate) and flt(obj.billed_amt))
            else flt(obj.qty)
        )
...
def is_subcontracting_order_created(po_name) -> bool:
    count = frappe.db.count(
        ...
        {"purchase_order": po_name, "status": ["not in", ["Draft", "Cancelled"]]},
    )
